...
import { createVehicles, getVehicles } from '../redux/feature/vehicleSlice.js';
...
export default function Farmer(){
    ...
    useEffect(() => {
          ...
          const fetchUsers = async () => {
               const response = await dispatch(getVehicles());
               console.log("Vehicle fetch: ",response)
               console.log("Vehicle fetch: ",response?.payload?.data)
	...
	const today = new Date()
     ...
     const onSubmit = async (values) => { 
          ...
          try{
               const response = await createVehicles(values)
               toast.current.show({severity: "success", summary: "Vehicle create Successfull" ,detail: response.data.message, life: 3000})
               ...
               console.log("Vehicle fetch: ",sortedProducts)
               ...
               console.log(err)
               toast.current.show({severity: "error", summary: "Vehicle creation Failed", detail: err.responseMessage, life: 3000})
    ...
    const header = (
          ...
          <h4 className="m-0">Manage Farmers</h4>
     ...
     return(
          <>
               <div>
                    <div className=" w-full bg-bg_dash md:w-120 lg:w-120 scrollbar-thin hover:scrollbar-thumb-gray-200 scrollbar-track-gray-300 h-[698px] overflow-auto mt-1">
                         <div>
                              <p className=" font-medium text-xl text-green-800 font-semibold py-6">Farmers</p>
                         </div>
                         ...
                         <DataTable ref={dt} value={farmers} selection={selectedFarmer} 
                              ...
                              header={header}>
                              <Column field="capacity" header="Number of seats" body={nameBodyTemplate} sortable ></Column>
                              <Column field="licenseNumber" header="License Number" sortable></Column>
                              <Column field="brand" header="Vehicle branch" sortable></Column>
          ...
          {/*                {product.image && <img src={`${product.image}`} alt={product.image} className="product-image rounded-md block m-auto pb-3" />}*/}
					...
					<div className=" rounded px-10 w-full m-auto" >
						...
						<h2 className="text-black block font-semibold text-3xl text-justify">Create Farmer</h2>
						<p className="text-createText mt-2 text-justify pb-4"> Enter your information to create an account</p>
							...
							<div>
								...
								{formik.errors.dob && formik.touched.dob && <p className="text-red-500 mt-2 text-sm">{formik.errors.dob}</p>}
                                   ...
                                   <div>
                                        ...
                                        <label className="text-black font-normal">Farming Type</label>
                                        <select
                                             value={formik.values.farmType}
                                             onBlur={formik.handleBlur("farmType")}
                                             onChange={formik.handleChange("farmType")}
                                             className="mt-1 border text-grey w-full text-md hover:shadow-md focus:shadow-md border-inputB rounded-md outline-none w-full h-10 pl-3"     >
                                             <option defaultValue>mixed</option>
                                             <option>Agriculture</option>
                                             <option>Livestock</option>
                                        </select>
                                        {formik.errors.farmType && formik.touched.farmType && <p className="text-red-500 mt-2 text-sm">{formik.errors.gender}</p>}
                                   </div>
                                   <div>
								<FormInput
									type="text"
									value={formik.values.farmlocate}
									onBlur={formik.handleBlur("farmlocate")}
									onChange={formik.handleChange("farmlocate")}
									placeholder="Farm location"
									label="Farm location"
								/>
								{formik.errors.farmlocate && formik.touched.farmlocate && <p className="text-red-500 mt-2 text-sm">{formik.errors.farmlocate}</p>}
							</div>
                                   <div>
                                        <FormInput
                                             type="password"
